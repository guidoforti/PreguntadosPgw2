...
function cambiarFiltro(nuevoFiltro) {
    ...
    document.querySelector(`[data-filtro="${nuevoFiltro}"]`).classList.add('active');
...
function cargarDatos(filtro) {
    ...
    cargarDatosGrafico('usuariosPorGrupoEdad', filtro, dibujarChartUsuariosPorGrupoEdad);
...
function cargarEstadisticasGenerales(filtro) {
    fetch(`/admin/obtenerDatosMetricas?metrica=estadisticasGenerales&filtro=${filtro}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(datos => {
            ...
            document.getElementById('totalJugadores').textContent = datos.totalJugadores || 0;
            document.getElementById('totalPartidas').textContent = datos.totalPartidasJugadas || 0;
            document.getElementById('totalPreguntasActivas').textContent = datos.totalPreguntasEnJuego || 0;
            document.getElementById('totalPreguntasCreadas').textContent = datos.totalPreguntasCreadas || 0;
            document.getElementById('usuariosNuevos').textContent = datos.usuariosNuevos || 0;
        })
        .catch(error => {
...
function dibujarChartUsuariosPorPais(datos) {
    if (!datos || datos.length === 0) {
        document.getElementById('chartUsuariosPorPais').innerHTML = '<p class="text-center text-muted">No hay datos disponibles</p>';
        return;
    }
    ...
    // Dibujar gráfico
    let chart = new google.visualization.ColumnChart(document.getElementById('chartUsuariosPorPais'));
...
function dibujarChartUsuariosPorSexo(datos) {
    if (!datos || datos.length === 0) {
        document.getElementById('chartUsuariosPorSexo').innerHTML = '<p class="text-center text-muted">No hay datos disponibles</p>';
        return;
    }
    ...
    // Dibujar gráfico
    let chart = new google.visualization.PieChart(document.getElementById('chartUsuariosPorSexo'));
    chart.draw(dataTable, options);
...
function dibujarChartUsuariosPorGrupoEdad(datos) {
    if (!datos || datos.length === 0) {
        document.getElementById('chartUsuariosPorGrupoEdad').innerHTML = '<p class="text-center text-muted">No hay datos disponibles</p>';
        return;
    }
    ...
    // Dibujar gráfico
    let chart = new google.visualization.BarChart(document.getElementById('chartUsuariosPorGrupoEdad'));
    chart.draw(dataTable, options);
...
function dibujarTablaRespuestas(datos) {
    ...
    if (!datos || datos.length === 0) {
        tbody.innerHTML = '<tr><td colspan="2" class="text-center text-muted">No hay datos disponibles</td></tr>';
        return;
    }
...
function actualizarTabla(tbodyId, datos, campos) {
    ...
    if (!datos || datos.length === 0) {
        let fila = document.createElement('tr');
        fila.innerHTML = '<td colspan="2" class="text-center text-muted">No hay datos disponibles</td>';
        tbody.appendChild(fila);
        return;
    }
